  ...
  tagList(
    fluidRow(
      shinydashboard::box(
        ...
        p("Outcomes table (scenario medians) for Springer Nature."),
      ...
      shinydashboard::box(
        ...
        solidHeader = TRUE, width = 6,
        p("KM survival curve at Springer 174mm width."),
...
mod_export_server <- function(id, sim_data) {
  moduleServer(id, function(input, output, session) {
    ...
    output$dl_latex_outcomes <- downloadHandler(
      ...
      content  = function(file) {
        tbl <- sim_data$summary |>
          ...
          dplyr::summarise(
            KIA_pct  = round(stats::median(.data$kia_rate,
                                           na.rm = TRUE) * 100, 1),
            IHL_med  = round(stats::median(.data$ihl_compliance_index,
                                           na.rm = TRUE), 3),
            n        = dplyr::n(),
            .groups  = "drop"
          )
    ...
    output$dl_figure_km <- downloadHandler(
      ...
      content  = function(file) {
        km <- dynasimR::km_estimate(sim_data, endpoint = "role2")
